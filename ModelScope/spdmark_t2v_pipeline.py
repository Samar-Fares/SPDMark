...
class TextToVideoSDPipeline(
    ...
    def decode_latents(self, latents, name, routing_mask_vae):
        """
        Decodes latents into frames.
        - latents: [B, C, T, H, W]
        - routing_mask_vae: [B, T, L, P]
        """
        # Unscale latents (same as diffusers)
        latents = latents / self.vae.config.scaling_factor
        B, C, T, H, W = latents.shape

        # ----- WATERMARKED DECODING -----
        if name == 'water':
            decoded = self.vae.decode_with_routing(latents, routing_mask_vae).sample   # [B*T,3,H,W]
        else:
            # normal decoding (framewise)
            latents_flat = latents.permute(0, 2, 1, 3, 4).reshape(B*T, C, H, W)
            decoded = self.vae.decode(latents_flat).sample                              # [B*T,3,H,W]

        # ----- RESHAPE TO VIDEO FORMAT -----
        video = (
            decoded.view(B, T, 3, decoded.shape[-2], decoded.shape[-1])
            .permute(0, 2, 1, 3, 4)
            .contiguous()
        )  # [B,3,T,H,W]
    ...
    def __call__(
        ...
        if output_type == "latent":
            ...
            video_tensor = self.decode_latents(latents, name, routing_mask_vae)
