...
def parse_args():
    ...
    parser.add_argument(
        "--exp_name",
        type=str,
        default="SPDMark_VAE_Res_Rank32_image",
    ...
    parser.add_argument(
        "--enable_attention_routing", action="store_true",
        help="Enable routed LoRA on VAE decoder attention projections (Q/K/V only)."
    )
...
def main():
    ...
    vae, num_resnet_blocks, num_attention_blocks, total_slots = inject_routing_blocks_into_video_vae_decoder(
        vae, num_paths=4, enable_attention=args.enable_attention_routing
    )
    ...
    print("RECALC num_train_epochs:", args.num_train_epochs)
    ...
    TOTAL_DECISIONS =  num_resnet_blocks + 3 * num_attention_blocks  
        ...
        for step, frames in enumerate(train_loader):
            ...
            with accelerator.accumulate(accelerator.unwrap_model(vae).decoder), accelerator.accumulate(extractor):
                ...
                route_mask = message_to_route_mask_video(
                    msg_bits, num_resnets=num_resnet_blocks, num_attns=num_attention_blocks,
                    num_paths=NUM_PATHS, per_block=True
                )
                print("msg_bits shape:", msg_bits.shape)


                target_bits = msg_bits  
                latents_bt_for_decode = latents_bt 
                ...
                out_routed = vae.decode_with_routing(
                    latents_bcthw, routing_mask=route_mask, num_frames=num_frames,
                    enable_attention=args.enable_attention_routing
                )
                frames_bt = out_routed.sample                 # [B*T, 3, H, W]
                generated_bcthw = frames_bt.view(bs, num_frames, 3, h, w).permute(0, 2, 1, 3, 4).contiguous()

                out_frozen = vae_frozen.decode(latents_bt_for_decode, num_frames=num_frames)
